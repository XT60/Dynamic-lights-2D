...
class Cell:
    def __init__(self, exist) -> None:
        ...
        self.has_wall = [False, False, False, False]  # determines if cell has wall in certain direction
        self.wall_i = [None, None, None, None]        # holds wall's indexes
...
class Tile_map:
    ...
    def convert_to_cell_map(self, tile_map):
        return [[Cell(val == 0) for val in tile_map[y]] for y in range(len(tile_map))]


    def reset_cell_map(self):
        for row in self.cell_map:
            for curr_cell in row:
                curr_cell.has_wall = [False, False, False, False]
                curr_cell.wall_i = [None, None, None, None] 
